% TODO: digitalsign already warns user.
...
\begin{figure}[htb]
	...
	\begin{subfigure}[b]{0.47\linewidth}
		\centering
		\includegraphics[width=\linewidth]{Screenshot 2026-05-28 at 23.44.27.png}
		\caption{Original signed contract.}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.463\linewidth}
		\centering
		%\raisebox{0.6ex}
		\includegraphics[width=\linewidth]{Screenshot 2026-05-28 at 23.44.42.png}
...
% TODO: add figure
% TODO: not exactly true, go back later. (looks true now...)
...
This paper demonstrates practical attacks against 14 signature verifiers, several of which are commercial products in wide use, including government facing and enterprise services. We disclose our findings through a coordinated process intended to reduce the exposure of users of the affected products while this research is made available to the community.

We have notified the vendor or maintainer of every verifier found vulnerable or partially vulnerable to ASA or SFDA under the criteria of Section~\ref{sec:winning-conditions}, and we are working with each of them toward a fix. Vendors for which we could not identify a direct contact were notified through the security or disclosure channel published on their website.

We follow a 90 day disclosure window per vendor, counted from the date that vendor was notified. A vendor that ships a fix before its window closes may request earlier disclosure. A vendor that has not responded by the time its window closes is disclosed regardless. Because not every vendor had shipped a fix by the time of this submission, we cannot yet publish a complete table of notification and disclosure dates for each vendor. We will publish the full list of affected verifiers and their fixed versions at \url{TODO} once every vendor has either shipped a fix or reached the end of its window, or, failing that, in an updated version of this paper before final publication. % TODO: fill in artifact/tracking URL before camera-ready

Consistent with this process, we distinguish what we release now from what we withhold. The test-case generation framework and the detection tool described in Section~\ref{sec:methodology} and Section~\ref{sec:countermeasures} are released as part of the SBSeg artifact submission, since neither tool signs or forges a document by itself. The code that assembles a concrete ASA or SFDA attack chain, along with the malicious PDF files produced during our evaluation, is withheld until the verifier it targets has shipped a fix or its disclosure window has closed.

We recognize that withholding these artifacts limits how directly other researchers can reproduce our results while some verifiers remain unpatched. We consider this an acceptable cost, since the released framework still lets a researcher regenerate equivalent test cases once their own trust anchors and target verifiers are ready, and publishing ready to use exploits for vulnerabilities that are not yet fixed would place users of the affected products at unnecessary risk.
